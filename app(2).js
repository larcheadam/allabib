...
async function fetchUserProfile(uid) {
  try {
    ...
    // If an Auth user exists but the public profile row is missing, rebuild it
    // from the Auth metadata only when the role is explicit and valid.
    if (!data && currentUser) {
      const userMeta = currentUser.user_metadata || {};
      const newName = userMeta.name || userMeta.full_name || (currentUser.email ? currentUser.email.split('@')[0] : 'User');
      const newRole = ['student', 'teacher', 'admin'].includes(userMeta.role) ? userMeta.role : null;
      ...
      if (newRole) {
        const { data: inserted, error: insertErr } = await sb
          ...
          .insert({
            ...
            role: newRole,
            class_id: newRole === 'student' ? (userMeta.class_id || null) : null
          })
          .select('*, classes(*)')
          .single();
    ...
    if (!data) throw new Error(currentLanguage === 'ar'
      ? 'الحساب موجود، لكن ملف المستخدم غير موجود في جدول profiles. أنشئ ملف المستخدم أولاً من لوحة الإدارة.'
      : 'The account exists, but its profiles record is missing. Create the user profile first.');
...
document.getElementById('login-form').addEventListener('submit', async (e) => {
  ...
  try {
    ...
    if (error) throw error;
    if (!data?.user) throw new Error('لم يتم إنشاء جلسة تسجيل الدخول');
...
async function loadAdminSubjectLists() {
  ...
  if (subjects) {
    subjects.forEach(sub => {
      ...
      const lvlName = sub.levels
        ? (currentLanguage === 'ar' ? sub.levels.name_ar : sub.levels.name_fr)
        : '--';
...
// CLASS CRUDS
const adminClassForm = document.getElementById('admin-class-form');
if (adminClassForm) adminClassForm.onsubmit = async (e) => {
  e.preventDefault();
  const levelId = document.getElementById('cls-level-select').value;
  const name = document.getElementById('cls-name').value.trim();
  if (!levelId || !name) {
    showToast(currentLanguage === 'ar' ? 'يرجى اختيار المستوى وكتابة اسم القسم' : 'Choose a level and enter a class name', 'warning');
    return;
  }
  const { error } = await sb.from('classes').insert({ level_id: Number(levelId), name });
  if (error) {
    showToast(error.message, 'danger');
    return;
  }
  showToast(currentLanguage === 'ar' ? 'تمت إضافة القسم بنجاح' : 'Class added successfully', 'success');
