...
// 5. Cloudinary Upload Helper (Unsigned upload)
...
async function loadStudentResources(classId) {
  ...
  if (files && files.length > 0) {
    files.forEach(file => {
      ...
      item.className = 'resource-item-card';
      const isImg = file.file_type === 'image';
      const icon = isImg ? 'fa-file-image text-warning' : 'fa-file-pdf text-danger';
      const subjectName = currentLanguage === 'ar' ? file.subjects.name_ar : file.subjects.name_fr;
      
      item.innerHTML = `
        <div class="res-icon ${isImg ? 'type-image' : ''}"><i class="fa-solid ${icon}"></i></div>
        <div class="res-info">
          <h5>${file.title}</h5>
          <span>${subjectName} - أستاذ ${file.profiles.name}</span>
          <a href="${file.file_url}" target="_blank" class="mt-2"><i class="fa-solid fa-circle-down"></i> ${TRANSLATIONS[currentLanguage].download}</a>
        </div>
      `;
...
document.getElementById('teacher-resource-form').onsubmit = async (e) => {
  ...
  try {
    const response = await uploadToCloudinary(file, resourceType, (percent) => {
    ...
    const { error } = await sb.from('resources').insert({
      ...
      file_type: fileType,
      file_public_id: response.publicId
    });
    
    if (error) throw error;
    
    showToast(currentLanguage === 'ar' ? "تم نشر الملف وتشاركه مع التلاميذ!" : "Resource shared successfully!", "success");
    document.getElementById('teacher-resource-form').reset();
  } catch (err) {
    console.error(err);
    showToast(currentLanguage === 'ar' ? "فشل رفع الملف" : "Failed to upload file", "danger");
  } finally {
...
document.getElementById('admin-level-form').onsubmit = async (e) => {
  e.preventDefault();
  const nameAr = document.getElementById('lvl-name-ar').value.trim();
  const nameFr = document.getElementById('lvl-name-fr').value.trim();
  
  const { error } = await sb.from('levels').insert({ name_ar: nameAr, name_fr: nameFr });
  if (!error) {
...
document.getElementById('admin-class-form').onsubmit = async (e) => {
  ...
  const { error } = await sb.from('classes').insert({ level_id: levelId, name: name });
  if (!error) {
    showToast(currentLanguage === 'ar' ? "تمت إضافة القسم بنجاح" : "Class added", "success");
...
window.deleteClass = async (id) => {
  if (confirm(currentLanguage === 'ar' ? "هل أنت متأكد من حذف هذا القسم نهائياً؟" : "Delete this class permanently?")) {
    const { error } = await sb.from('classes').delete().eq('id', id);
    if (!error) {
      showToast(currentLanguage === 'ar' ? "تم حذف القسم" : "Class deleted", "success");
      await loadAdminDashboard();
    }
  }
};
...
document.getElementById('admin-subject-form').onsubmit = async (e) => {
  ...
  const nameAr = document.getElementById('subj-name-ar').value.trim();
  const nameFr = document.getElementById('subj-name-fr').value.trim();
...
window.showQrModal = (name, role, className, qrToken) => {
  ...
  QRCode.toCanvas(qrToken || 'allabib_error', { width: 140 }, function (err, canvas) {
    if (err) console.error(err);
    else canvasTarget.appendChild(canvas);
  });
...
document.getElementById('admin-summons-form').onsubmit = async (e) => {
  ...
  const reason = document.getElementById('sum-reason-ar').value.trim();
  const reasonFr = document.getElementById('sum-reason-fr').value.trim();
...
async function loadAdminSummonsLogs() {
  ...
  if (summons) {
    summons.forEach(s => {
      const row = document.createElement('tr');
      const reasonText = currentLanguage === 'ar' ? s.reason : s.reason_fr;
...
document.getElementById('admin-announcement-form').onsubmit = async (e) => {
  ...
  const titleFr = document.getElementById('ann-title-fr').value.trim();
  const msgAr = document.getElementById('ann-msg-ar').value.trim();
  const msgFr = document.getElementById('ann-msg-fr').value.trim();
  ...
  if (!error) {
    showToast(currentLanguage === 'ar' ? "تم نشر الإعلان وتوزيعه بنجاح!" : "Announcement broadcasted!", "success");
...
async function loadNotificationsDropdown() {
  ...
  if (list && list.length > 0) {
    list.forEach(notif => {
      ...
      const titleText = currentLanguage === 'ar' ? notif.title : notif.title_fr;
      const msgText = currentLanguage === 'ar' ? notif.message : notif.message_fr;
