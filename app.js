...
async function loadStudentExams(classId) {
  ...
  if (!container) return;
  if (!classId) {
    container.innerHTML = `<p class="empty-notif">${currentLanguage === 'ar' ? 'لم يتم تحديد القسم' : 'Aucune classe spécifiée'}</p>`;
    return;
  }
  ...
  if (error) {
    ...
    container.innerHTML = `<p class="empty-notif">${currentLanguage === 'ar' ? 'تعذر تحميل الامتحانات' : 'Erreur de chargement'}</p>`;
    return;
  }

  if (!exams || exams.length === 0) {
    container.innerHTML = `<p class="empty-notif"><i class="fa-solid fa-calendar-check text-muted"></i> ${currentLanguage === 'ar' ? 'لا توجد امتحانات مبرمجة حالياً' : 'Aucun examen programmé'}</p>`;
    return;
  }
  ...
  container.innerHTML = exams.map(e => {
    ...
    let daysBadge = '';
    if (diffDays === 0) {
      daysBadge = `<span class="badge badge-danger animate-pulse">${currentLanguage === 'ar' ? 'اليوم' : "Aujourd'hui"}</span>`;
    } else if (diffDays === 1) {
      daysBadge = `<span class="badge badge-warning">${currentLanguage === 'ar' ? 'غداً' : 'Demain'}</span>`;
    } else {
      daysBadge = `<span class="badge badge-primary">${currentLanguage === 'ar' ? `متبقي ${diffDays} أيام` : `Dans ${diffDays} jours`}</span>`;
    }

    const subjName = e.subjects ? (currentLanguage === 'ar' ? e.subjects.name_ar : e.subjects.name_fr) : (e.title || '');
    const teacherName = e.profiles ? e.profiles.name : '';
    const timeRange = e.start_time ? `${e.start_time.substring(0, 5)} - ${(e.end_time || '').substring(0, 5)}` : '';
    const roomInfo = e.room_number ? `<span class="exam-room"><i class="fa-solid fa-door-open"></i> ${currentLanguage === 'ar' ? 'القاعة' : 'Salle'} ${e.room_number}</span>` : '';

    return `
      <div class="exam-card card-nested shadow-sm">
        <div class="exam-card-header">
          <div class="exam-title-group">
            <h4 class="exam-title">${e.title || subjName}</h4>
            <span class="exam-subj-tag">${subjName}</span>
          </div>
          ${daysBadge}
        </div>
        <div class="exam-card-body">
          <p><i class="fa-solid fa-calendar-day"></i> <strong>${e.exam_date}</strong> ${timeRange ? `(${timeRange})` : ''}</p>
          ${teacherName ? `<p><i class="fa-solid fa-chalkboard-user"></i> ${currentLanguage === 'ar' ? 'الأستاذ:' : 'Enseignant:'} ${teacherName}</p>` : ''}
          ${roomInfo ? `<p>${roomInfo}</p>` : ''}
          ${e.notes ? `<p class="exam-notes text-muted"><i class="fa-solid fa-note-sticky"></i> ${e.notes}</p>` : ''}
        </div>
      </div>
    `;
  }).join('');
...
async function loadTeacherExams() {
  const list = document.getElementById('teacher-exams-list');
  if (!list) return;
  ...
  list.innerHTML = (data || []).map(e => {
    const subjName = currentLanguage === 'ar' ? e.subjects?.name_ar : e.subjects?.name_fr;
    const displayName = e.title || subjName;
    const examDateTime = `${e.exam_date} ${e.start_time.substring(0, 5)}`;
    return `
      <div class="admin-announcement-item">
        <div>
          <strong>${displayName}</strong>
          <small>${e.classes?.name || ''} · ${examDateTime}</small>
        </div>
        <button class="btn btn-danger btn-sm" onclick="deleteExamSchedule(${e.id})">
          <i class="fa-solid fa-trash"></i> ${currentLanguage === 'ar' ? 'حذف' : 'Supprimer'}
        </button>
      </div>
    `;
  }).join('') || `<p class="empty-notif">${currentLanguage === 'ar' ? 'لم يتم نشر مواعيد اختبارات بعد.' : 'Aucun examen planifié.'}</p>`;
}

const teacherExamForm = document.getElementById('teacher-exam-form');
if (teacherExamForm) {
  teacherExamForm.onsubmit = async (e) => {
    e.preventDefault();
    const classId = Number(document.getElementById('exam-class-select').value);
    const subjectId = Number(document.getElementById('exam-subject-select').value);
    const dateVal = document.getElementById('exam-date').value;
    const titleInput = document.getElementById('exam-title').value.trim();
    
    if (!classId || !subjectId || !dateVal) {
      showToast(currentLanguage === 'ar' ? "يرجى ملء جميع الحقول المطلوبة" : "Please fill all required fields", "warning");
      return;
    }
    
    const startDt = new Date(dateVal);
    const endDt = new Date(startDt.getTime() + 2 * 60 * 60 * 1000); // 2 hours duration
    
    const exam_date = dateVal.split('T')[0];
    const start_time = dateVal.split('T')[1] + ':00';
    const end_time = endDt.toISOString().split('T')[1].substring(0, 8);
    const title = titleInput || (currentLanguage === 'ar' ? 'اختبار' : 'Examen');
    ...
    const { error } = await sb.from('exam_schedules').insert(payload);
    if (error) {
      return showToast(error.message, 'danger');
    }
    
    showToast(currentLanguage === 'ar' ? 'تم نشر موعد الاختبار بنجاح.' : 'Exam schedule published successfully.', 'success');
    teacherExamForm.reset();
    await loadTeacherExams();
  };
}

window.deleteExamSchedule = async (id) => {
  if (!confirm(currentLanguage === 'ar' ? 'هل تريد حذف موعد الاختبار هذا؟' : 'Supprimer cet examen ?')) return;
  const { error } = await sb.from('exam_schedules').delete().eq('id', id);
  if (error) {
    showToast(currentLanguage === 'ar' ? 'فشل حذف موعد الاختبار' : 'Failed to delete exam', 'danger');
  } else {
    showToast(currentLanguage === 'ar' ? 'تم حذف موعد الاختبار بنجاح' : 'Exam deleted successfully', 'success');
    await loadTeacherExams();
  }
};
...
window.deleteResourceItem = async (id) => {
  if (!confirm(currentLanguage === 'ar' ? 'هل تريد حذف هذا المورد التعليمي؟' : 'Supprimer cette ressource ?')) return;
  const { error } = await sb.from('resources').delete().eq('id', id);
  if (error) {
    showToast('فشل حذف المورد', 'danger');
  } else {
    showToast('تم حذف المورد التعليمي بنجاح', 'success');
    if (typeof loadTeacherResources === 'function') loadTeacherResources();
  }
};

window.deleteAnnouncementItem = async (id) => {
  if (!confirm(currentLanguage === 'ar' ? 'هل تريد حذف هذا الإعلان؟' : 'Supprimer cette annonce ?')) return;
  const { error } = await sb.from('notifications').delete().eq('id', id);
  if (error) {
    showToast('فشل حذف الإعلان', 'danger');
  } else {
...
window.deleteSummonsItem = async (id) => {
  if (!confirm(currentLanguage === 'ar' ? 'هل تريد إلغاء هذا الاستدعاء؟' : 'Annuler cette convocation ?')) return;
  const { error } = await sb.from('parent_summons').delete().eq('id', id);
  if (error) {
    showToast('فشل إلغاء الاستدعاء', 'danger');
  } else {
    showToast('تم إلغاء الاستدعاء بنجاح', 'success');
    if (typeof loadAdminSummons === 'function') loadAdminSummons();
  }
};
